...
-- Break tracking: retain the shift interval but deduct recorded break time from
-- the generated worked duration used by dashboards, reports, and exports.
alter table public.time_entries
  add column if not exists break_started_at timestamptz,
  add column if not exists break_seconds integer not null default 0;
alter table public.time_entries drop constraint if exists time_entries_break_seconds_check;
alter table public.time_entries
