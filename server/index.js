...
app.delete('/v1/time-entries/:id', authenticate, adminOnly, async (req, res, next) => { try { const entry = await query(db.from('time_entries').delete().eq('id', req.params.id).select().single()); await audit(req, 'DELETE', 'TIME_ENTRY', entry.id, 'Deleted time entry'); res.json(entry); } catch (error) { next(error); } });
