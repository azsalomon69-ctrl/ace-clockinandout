...
async function loadDeletedTimeEntries() {
  ...
  try {
    ...
    body.innerHTML = entries.length ? entries.map(entry => '<tr><td>' + deletedEntryEscape(entry.profiles?.full_name || entry.profiles?.email || 'Unknown') + '</td><td>' + deletedEntryEscape(entry.projects?.name || 'No project') + '</td><td>' + new Date(entry.clock_in_at).toLocaleString() + '</td><td>' + deletedEntryDuration(entry.duration_seconds) + '</td><td><button class="btn btn-sm btn-outline restore-entry" data-id="' + entry.id + '" type="button">Restore</button></td></tr>').join('') : '<tr><td colspan="5">No deleted time entries.</td></tr>';
