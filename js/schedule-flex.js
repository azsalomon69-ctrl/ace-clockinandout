(() => {
  ...
  const load = async () => {
    ...
    document.getElementById('scheduleEmployee').innerHTML = employees.map(user => `<option value="${user.id}">${user.full_name || user.email}</option>`).join('');
    document.getElementById('scheduleAssignment').innerHTML = schedules.filter(item => item.is_active).map(item => `<option value="${item.id}">${item.name}</option>`).join('');
    document.getElementById('scheduleRows').innerHTML = schedules.length ? schedules.map(item => `<tr><td>${item.name}</td><td>${item.schedule_type === 'FLEX' ? 'Flextime' : `${item.start_time?.slice(0,5)}–${item.end_time?.slice(0,5)}`}</td><td>${item.daily_elapsed_minutes / 60}h</td><td>${item.break_limit_minutes}m</td><td>${item.user_schedule_assignments?.length || 0}</td></tr>`).join('') : '<tr><td colspan="5">No schedules yet.</td></tr>';
  };
