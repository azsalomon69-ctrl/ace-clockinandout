...
(() => {
  ...
  async function loadChatLog() {
    ...
    try {
      ...
      list.innerHTML = messages.length ? messages.map(message => `<article class="chat-log-entry"><div class="chat-log-entry-head"><strong>${escape(name(message.sender))}</strong><span>to</span><strong>${escape(name(message.recipient))}</strong><time>${dateTime(message.created_at)}</time></div><p>${escape(message.body)}</p>${message.deleted_at ? '<small>Deleted in employee chat</small>' : message.edited_at ? '<small>Edited</small>' : ''}</article>`).join('') : '<div class="chat-log-empty"><span><img class="ui-icon" src="assets/icons/message-circle-more.svg" alt="" aria-hidden="true"></span><strong>No employee messages yet</strong><p>Private employee conversations will appear here for administrative review.</p></div>';
